...
- 2画面目の座標を選択していて、1画面の環境からRDPで接続したり、
  モニタを外したりすると指定の座標が画面の外になったりするので、
  そのときはうまくまるめたい。

### 起動時のディレクトリをコマンドライン以外で設定できるようにしたい
- 例えばWindowsでタスクバーにnmf.exeを固定して、起動するとnmf.exeの存在するディレクトリを対象にnmfが開くが、
  それはあまりうれしくないので……。
- コマンドラインで指定されたらそっちを優先するとよい。

### デバッグログの強化
- config.jsonかinit.starでログディレクトリを設定できるようにする。
  デフォルトはconfig.jsonとinit.starと同じ場所にlogsフォルダを掘ることにする。
- config.jsonかinit.starでデバッグログを有効化できるようにする。
- デバッグログが有効な場合は、起動毎に新しいログファイルを作る。
  指定の個数(これもconfig.jsonかinit.starで設定する)を超えたら古いファイルから消える。
- ようは常時デバッグログを出しておけるようにして、妙な現象を見たら、ログを有効化してずっと使って、
  状況を再現できるようにしたい。
- 妙な現象としては、キーが効かなくなるというのがありがちなので、デバッグログ有効時はtoolbarに
  アイコンを足してそれをクリックすると、keymanagerの状況をダンプできると助けになるかも。
  (キーが効かなくときでもマウスにはだいたい反応するので……)
